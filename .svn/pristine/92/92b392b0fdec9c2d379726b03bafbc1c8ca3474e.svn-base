...
public class SplitPlotCompletelyRandomizedDesignDialog extends Dialog {
	...
	private Label lblMainPlotFactor;
	private Label lblSubplotFactor;
	private Label lblNumberOfTrials;
	 ...
	 * @param parentShell
	 */
	public SplitPlotCompletelyRandomizedDesignDialog(Shell parentShell) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite parent) {
		...
		lblFactorialDesign.setText("Split Plot in CRD");
		...
		gd_grpDesignParameters.heightHint = 234;
		...
		lblMainPlotFactor = new Label(group, SWT.NONE);
		lblMainPlotFactor.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true, false, 1, 1));
		lblMainPlotFactor.setText("Mainplot");
		...
		lblSubplotFactor = new Label(group, SWT.NONE);
		lblSubplotFactor.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true, false, 1, 1));
		lblSubplotFactor.setText("Subplot");
		...
		lblNumberOfReplicates = new Label(composite_1, SWT.NONE);
		lblNumberOfReplicates.setFont(SWTResourceManager.getFont("Tahoma", 8, SWT.NORMAL));
		lblNumberOfReplicates.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		...
		lblNumberOfRows.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		...
		lblNumberOfField = new Label(composite_1, SWT.NONE);
		lblNumberOfField.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		...
		lblNumberOfTrials = new Label(composite_1, SWT.NONE);
		lblNumberOfTrials.setFont(SWTResourceManager.getFont("Tahoma", 8, SWT.NORMAL));
		lblNumberOfTrials.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		...
		txtFileName.setText("fieldbookSplitCRD");
...
//	}
	...
	if(txtMainPlotFactorID.getCharCount()>4 || txtSubPlotFactorID.getCharCount()>4){
		MessageDialog.open(SWT.ERROR, this.getShell(), "Error", "Factor ID must contain not more than four characters. ", SWT.ERROR);
		return;	
	}
	
	int plot = (txtTotalReplicates.getSelection() *  txtMainPlotFactorLevels.getSelection() * txtSubPlotFactorLevels.getSelection());
	...
	if ((txtFieldRows.getSelection()  % txtRows.getSelection()) !=0 ){
		MessageDialog.openError(getShell(), "Error", "Number of field rows must be divisible by the number of plots along the length of each main plot."); 
		return ;
	}
	
	if (txtRows.getSelection() > txtFieldRows.getSelection()){
		MessageDialog.openError(getShell(), "Error", "Number of plots along the length of each main plot should be less than or equal to the number of field rows and should divide the number of field rows."); 
		return ;
	}
...
//	String outputFile = StarRandomizationUtilities.createOutputFile("SplitPlotCRD", txtFileName.getText());
//	String outputFileTxt = outputFile + ".txt";
//	String outputFileCsv = outputFile + ".csv";
	
	String outputFile = StarRandomizationUtilities.createOutputFolder("SplitPlotCRD");
...
//			txtRows.getSelection(),
//			txtFieldRows.getSelection(),
//			txtTotalTrials.getSelection(), "crd");
