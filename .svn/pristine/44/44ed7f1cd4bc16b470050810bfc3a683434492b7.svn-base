...
public class SplitSplitPlotCompletelyRandomizedDesignDialog extends Dialog {
	private Spinner txtTotalReplicates;
	private Spinner txtTotalTrials;
	private Label lblFactorialDesign;
	 ...
	 * @param parentShell
	 */
	public SplitSplitPlotCompletelyRandomizedDesignDialog(Shell parentShell) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite parent) {
		...
		lblFactorialDesign = new Label(container, SWT.NONE);
		...
		lblFactorialDesign.setText("Split-Split Plot in CRD");
		...
		gd_grpDesignParameters.heightHint = 244;
		...
		composite_1.setFont(SWTResourceManager.getFont("Tahoma", 8, SWT.NORMAL));
		...
		txtFileName.setText("fieldbookSplit2CRD");
	...
	protected void okPressed(){  	
		...
		int plot = (txtTotalReplicates.getSelection() *  txtMainPlotFactorLevels.getSelection() * txtSubPlotFactorLevels.getSelection() * txtSubSubPlotFactorLevels.getSelection());
		...
		if ((txtFieldRows.getSelection()  % (txtRows.getSelection() * txtRowsPerSub.getSelection())) !=0 ){
			MessageDialog.openError(getShell(), "Error", "Number of field rows must be divisible by the number of plots along the length of each main plot and number of plots along the length of each sub plot."); 
			return ;
		}
		
		if ((txtRows.getSelection() * txtRowsPerSub.getSelection()) > txtFieldRows.getSelection()){
			MessageDialog.openError(getShell(), "Error", " The product of the number of plots along the length of each main plot and rows per sub plot should be less than or equal to the number of field rows and should divide the number of field rows."); 
			return ;
		}
		...
		String outputFile = StarRandomizationUtilities.createOutputFolder("SplitSplitPlotCRD");
