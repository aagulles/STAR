...
//				lstResponse, btnResponseVariable);
		...
		listManager.initializeSingleButtonList(lstFactorVariables, lstFactors,btnMove, btnFactors);
	...
	protected void okPressed() {
		...
		if (validateCmpModel()) {
			...
			StarAnalysisUtilities.testVarArgs(filePath.replace(File.separator, "/"),
					outputFolderPath.replace(File.separator, "/"), design,
					lstResponse.getItems(), lstFactors.getItems(), null, null,
					null, txtBlock.getText(), null, btnDisplay1.getSelection(),
					btnDisplay2.getSelection(), btnDisplay3.getSelection(),
					postHocVar, postHocOpts, isContrastReady,
					(double) txtSignificantLevel.getSelection() / 100);
