...
//				}
	...
	public void initializeSingleButtonList(final List lstFrom, final List lstTo, final Button btnMove, final Button btnAddRemove){
		...
		lstFrom.addMouseListener(new MouseListener(){
			...
			public void mouseDoubleClick(MouseEvent e) {
				// TODO Auto-generated method stub
				System.out.println(btnMove.getText());
				btnMove.setEnabled(false);
				System.out.println(btnMove.isEnabled());
				exchangeList(lstFrom,lstTo);
				btnAddRemove.setEnabled(false);
...
//					if(!btnAddRemove.getText().equals("Remove")){
		...
		btnAddRemove.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				if(lstFrom.getSelectionCount() > 1){
					MessageDialog.openError(Display.getCurrent().getActiveShell(), "Error", "Please select one item only.");
					return;
				}
				if(btnAddRemove.getText().equals("Add") && lstFrom.getSelectionCount() > 0 ){
...
//					btnAddRemove.setEnabled(false);
	...
	public void initializeSingleSelectionList(final List lstFrom, final Text txtTo, final Button btnMove, final Button btnAddRemove){
		txtTo.setEditable(false);
		btnAddRemove.setEnabled(false);
		btnMove.setEnabled(false);
		lstFrom.addSelectionListener(new SelectionListener(){
...
//					btnAddRemove.setEnabled(false);
	...
	public void initializeSingleSelectionList(final List lstFrom, final Text txtTo, final Button btnMove, final Button btnAddRemove, final DialogListValidator lstValidator){
		txtTo.setEditable(false);
		txtTo.setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));
		btnAddRemove.setEnabled(false);
		btnMove.setEnabled(false);
