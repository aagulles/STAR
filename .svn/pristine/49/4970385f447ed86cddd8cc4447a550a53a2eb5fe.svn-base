...
public class OneStageMultiEnvironmentHandler {
	ProjectTreeComponent	projectTreeComponent ;
	OneStageMultiEnvironmentDialog oneStageMultiEnvironmentDialog;
	@Inject
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		dataManipulationManager.saveFileChanges(new File(absoluteFilePath), DataManipulationManager.getActiveTable(absoluteFilePath),  DataManipulationManager.getActiveDataDelimiter(absoluteFilePath));
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("oneStageMultiEnv", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("oneStageMultiEnv",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					oneStageMultiEnvironmentDialog = new OneStageMultiEnvironmentDialog(parent, "oneStageMultiEnv", activeFile);
					oneStageMultiEnvironmentDialog.open();
				}
			}
