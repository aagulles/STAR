...
public class DiallelGriffingIHandler {
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		try{//check if there's an active File first
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("GriffingI", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("GriffingI",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					griffingMethodDialog = new GriffingMethodDialog(parent, "GriffingI", activeFile);
