...
import org.irri.breedingtool.graphs.dialog.HistogramDialog;
...
public class LineGraphHandler {
	...
	public static void execute() {
		try{
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("LineGraph", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("LineGraph",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another

					LineGraphDialog newDialog = new LineGraphDialog(Display.getCurrent().getActiveShell(), "LineGraph", newFileModel.getProjectFile());
					newDialog.open();
				}
			}
