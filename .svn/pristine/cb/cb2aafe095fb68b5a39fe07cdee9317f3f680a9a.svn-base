...
public class SelectionIndexHandler {
	ProjectTreeComponent	projectTreeComponent ;
	SelectionIndexDialog selectionIndexDialog;
	@Inject
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		dataManipulationManager.saveFileChanges(new File(absoluteFilePath), DataManipulationManager.getActiveTable(absoluteFilePath),  DataManipulationManager.getActiveDataDelimiter(absoluteFilePath));
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("selectionIndex", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("selectionIndex",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					selectionIndexDialog = new SelectionIndexDialog(parent, "selectionIndex", activeFile);
					selectionIndexDialog.open();
				}
			}
