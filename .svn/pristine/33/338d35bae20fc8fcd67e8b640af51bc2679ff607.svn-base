package org.irri.breedingtool.pbtools.analysis.matingdesign.handler;
...
import org.irri.breedingtool.pbtools.analysis.matingdesign.dialog.TripleTestCrossDialog;
...
public class TripleTestCrossHandler {
	ProjectTreeComponent projectTreeComponent ;
	TripleTestCrossDialog tripleTestCrossDialog;
	@Inject
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		try{//check if there's an active File first
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("TripleTestCross", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("TripleTestCross",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					tripleTestCrossDialog = new TripleTestCrossDialog(parent, "TripleTestCross", activeFile);
					tripleTestCrossDialog.open();
				}
			}
