...
import org.irri.breedingtool.pbtools.analysis.generationmean.dialog.RawDataDialog;
...
public class RawDataDialogHandler {
	ProjectTreeComponent	projectTreeComponent ;
	RawDataDialog newDialog;
	@Inject
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		dataManipulationManager.saveFileChanges(new File(absoluteFilePath), DataManipulationManager.getActiveTable(absoluteFilePath),  DataManipulationManager.getActiveDataDelimiter(absoluteFilePath));
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("GMARawData", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("GMARawData",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					newDialog = new RawDataDialog(parent, "GMARawData", activeFile);
					newDialog.open();
				}
			}
