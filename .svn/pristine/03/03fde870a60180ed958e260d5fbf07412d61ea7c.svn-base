...
import org.irri.breedingtool.datamanipulation.dialog.AppendDialog;
...
public class AppendHandler {
	...
	public static void execute() {
		try{
			...
			AppendDialog newDialog = new AppendDialog(Display.getCurrent().getActiveShell(), newFileModel);
			newDialog.open();
	...
	public static void execute(ProjectExplorerTreeNodeModel newFileModel, int fileFormat, String delimiter) {
		...
		ArrayList<String> varInfo=dataManipulationManager.getVarInfoFromFile(absoluteFilePath);
		System.out.println(newFileModel.getProjectFile().getName());
		AppendDialog newVarDlg = new AppendDialog(Display.getCurrent().getActiveShell(), newFileModel);
		newVarDlg.open();
	}
}
