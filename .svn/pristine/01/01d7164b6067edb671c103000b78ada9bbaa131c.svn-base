...
public class DiallelGriffingIVHandler {
	ProjectTreeComponent	projectTreeComponent ;
	GriffingMethodDialog griffingMethodDialog;
	@Inject
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		try{//check if there's an active File first
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("GriffingIV", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("GriffingIV",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					griffingMethodDialog = new GriffingMethodDialog(parent, "GriffingIV", activeFile);
					griffingMethodDialog.open();
				}
			}
