...
public class NorthCarolinaExperimentIIHandler {
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		try{//check if there's an active File first
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("NCII", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("NCII",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					matingDesignDialog = new NorthCarolinaExperimentsDialog(parent, "NCII", activeFile);
