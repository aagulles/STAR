...
//								txtTotalTreatments.setMinimum(9);
	...
	protected void okPressed(){ 
		...
		if(txtTotalTreatments.getSelection() < 9 || txtTotalTreatments.getSelection() > 144){
			MessageDialog.openError(getShell(), "Error", "The minimum number of treatments allowed is 9 and the maximum is 144.");
			txtTotalTreatments.setSelection(9);
			return;
		}
		...
		if(txtFieldRows.getSelection()<3){
			MessageDialog.openError(getShell(), "Error", "The minimum value of the number of field rows is equal to 3."); 
			txtFieldRows.setSelection(3);
			return;
		}
		...
		if(!isPerfectSquare(txtTotalTreatments.getSelection())){
			MessageDialog.openError(getShell(), "Error", "The number of treatments must be a perfect square"); 
			return ; 
		}
		if(Math.sqrt(txtTotalTreatments.getSelection()) % 2 == 0){
			if(Math.sqrt(txtTotalTreatments.getSelection()) == 4){
					...
					MessageDialog.openError(getShell(), "Error", "The number of replicates must be an integer from 2 to 5."); 
					return;		
					}
			}
				else{
					...
					if(txtTotalReplicates.getSelection() >  3){
						System.out.println("if T=" + txtTotalTreatments.getSelection() + " | " + txtTotalReplicates.getSelection());
						MessageDialog.openError(getShell(), "Error", "The number of replicates can either be 2 or 3."); 
						return;		
					}	
				}
			...
			if(Math.sqrt(txtTotalTreatments.getSelection()) + 1 < txtTotalReplicates.getSelection()){
				MessageDialog.openError(getShell(), "Error", "The number of replicates must be an integer from 2 to (" + (int) (Math.sqrt(txtTotalTreatments.getSelection()) + 1) + ")"); 
				return;		
			}
		}

		if ((txtFieldRows.getSelection()  % Math.sqrt(txtTotalTreatments.getSelection()) !=0 )){
			MessageDialog.openError(getShell(), "Error", "The number of field rows must be divisible by the square root of the number of treatments."); 
			return ;
		}
		
		int number = txtTotalReplicates.getSelection();
		boolean isFactor = false;
		for (int i = 1; i <= number; i++) { 
			if (number % i == 0) { 			
				if(txtFieldRows.getSelection()== ( i* Math.sqrt(txtTotalTreatments.getSelection()))){
					isFactor = true;
					break;	
					}
				} 		
		...
		if(!isFactor){
		MessageDialog.openError(getShell(), "Error", "The number of field rows must be equal to a * k where a is a factor of the number of replicates (" + txtTotalReplicates.getSelection() + ") and k is the square root of the number of treatments."); 			
		}
		else{
		...
		String outputFile = StarRandomizationUtilities.createOutputFolder("Lattice");
