...
import org.irri.breedingtool.pbtools.analysis.environment.dialog.MultiplicativeModelsNewDialog;
...
public class MultiplicativeModelsHandler {
	ProjectTreeComponent	projectTreeComponent ;
	MultiplicativeModelsNewDialog multiplicativeModelsNewDialog;
	@Inject
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		dataManipulationManager.saveFileChanges(new File(absoluteFilePath), DataManipulationManager.getActiveTable(absoluteFilePath),  DataManipulationManager.getActiveDataDelimiter(absoluteFilePath));
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("MultiplicativeModels", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("MultiplicativeModels",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					multiplicativeModelsNewDialog = new MultiplicativeModelsNewDialog(parent, "MultiplicativeModels", activeFile);
					multiplicativeModelsNewDialog.open();
				}
			}
