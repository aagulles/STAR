...
public class SplitSplitSplitRandomizedCompleteBlockDesignDialog extends Dialog {
	 ...
	 * @param parentShell
	 */
	public SplitSplitSplitRandomizedCompleteBlockDesignDialog(Shell parentShell) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite parent) {
		...
		lblFactorialDesign.setText("Split-Split-Split Plot in RCBD");
		...
		gd_grpDesignParameters.heightHint = 349;
		...
		gd_txtSubSubSubPlotFactorID.widthHint = 50;
		...
		Label lblNumberOfRows_3 = new Label(composite_1, SWT.NONE);
		lblNumberOfRows_3.setText("Number of Sub-sub-subplots along the Length of Each Sub-subplot");
		lblNumberOfRows_3.setFont(SWTResourceManager.getFont("Tahoma", 8, SWT.NORMAL));
		new Label(composite_1, SWT.NONE);
		...
		txtFileName.setText("fieldbookSplit3RCB");
	...
	protected void okPressed(){  	
		...
		int plot = (txtTotalBlocks.getSelection() *  txtMainPlotFactorLevels.getSelection() * txtSubPlotFactorLevels.getSelection() * txtSubSubPlotFactorLevels.getSelection() * txtSubSubSubPlotFactorLevels.getSelection());
		...
		if ((txtFieldRows.getSelection() % (txtRowsPerBlk.getSelection()* txtRows.getSelection()*txtRowsPerSub.getSelection() *txtRowsPerSubSub.getSelection())) !=0 ){
			MessageDialog.openError(getShell(), "Error", "Number of field rows must be divisible by the product of the number of main plots along the length of each block ,number of plots along the length of each main plot, along the length of each sub plot and along the length of each sub-sub plot."); 
			return ;
		}
		
		if ((txtRowsPerBlk.getSelection()* txtRows.getSelection()*txtRowsPerSub.getSelection()*txtRowsPerSubSub.getSelection()) > txtFieldRows.getSelection()){
			MessageDialog.openError(getShell(), "Error", "The product of the number of main plots along the length of each block ,number of plots along the length of each main plot, along the length of each sub plot and along the length of each sub-sub plot should be less than or equal to the number of field rows and should divide the number of field rows."); 
			return ;
		}
		...
		String outputFile = StarRandomizationUtilities.createOutputFolder("SplitSplitSplitPlotRCBD");
