...
public class SplitSplitPlotLatinSquareDesignDialog extends Dialog {
	...
	protected void okPressed(){  	
		...
		if (txtRows.getSelection() > txtSubPlotFactorLevels.getSelection()){
			MessageDialog.open(SWT.ERROR, this.getShell(), "Error", "Number of plots along the length of each main plot should be less than or equal to the levels of the sub plot factor and must divide the levels of the sub plot factor", SWT.ERROR);
			return;
		}
		...
		if (txtRowsPerSub.getSelection() > txtSubSubPlotFactorLevels.getSelection()){
			MessageDialog.open(SWT.ERROR, this.getShell(), "Error", "Number of row per sub plot should be less than or equal to the levels of the sub-sub plot factor and must divider the levels of the sub-sub plot factor", SWT.ERROR);
			return;
		}
