...
public class DiallelGriffingIIHandler {
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		try{//check if there's an active File first
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("GriffingII", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("GriffingII",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					griffingMethodDialog = new GriffingMethodDialog(parent, "GriffingII", activeFile);
