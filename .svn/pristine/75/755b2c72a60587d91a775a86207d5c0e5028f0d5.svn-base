...
public class NorthCarolinaExperimentIIIHandler {
	ProjectTreeComponent	projectTreeComponent ;
	NorthCarolinaExperimentsDialog matingDesignDialog;
	@Inject
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		try{//check if there's an active File first
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("NCIII", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("NCIII",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					matingDesignDialog = new NorthCarolinaExperimentsDialog(parent, "NCIII", activeFile);
					matingDesignDialog.open();
				}
			}
