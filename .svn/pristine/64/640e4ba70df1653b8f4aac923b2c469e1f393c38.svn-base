...
public class SplitPlotRandomizedCompleteBlockDesignDialog extends Dialog {
	...
	protected void okPressed(){  	
		...
		if ((txtFieldRows.getSelection() % (txtRowsPerBlk.getSelection()* txtRows.getSelection())) !=0 ){
			MessageDialog.openError(getShell(), "Error", "Number of field rows must be divisible by the product of the number of main plots along the length of each block and number of plots along the length of each main plot."); 
			return ;
		}
		
		if ((txtMainPlotFactorLevels.getSelection() % txtRowsPerBlk.getSelection()) !=0 ){
			MessageDialog.openError(getShell(), "Error", "Number of main plots along the length of each block must divide the levels of the main plot factor."); 
			return ;
		}
		
		if ((txtRowsPerBlk.getSelection()* txtRows.getSelection()) > txtFieldRows.getSelection()){
			MessageDialog.openError(getShell(), "Error", "The product of the number of main plots along the length of each block and number of plots along the length of each main plot should be less than or equal to the number of field rows and should divide the number of field rows."); 
			return ;
		}
		...
		String outputFile = StarRandomizationUtilities.createOutputFolder("SplitPlotRCBD");
