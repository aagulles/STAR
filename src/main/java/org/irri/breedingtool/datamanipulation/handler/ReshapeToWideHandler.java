...
public class ReshapeToWideHandler {
	...
	public static void execute() {
		try{
			...
			ReshapeLongToWideDialog newDialog = new ReshapeLongToWideDialog(Display.getCurrent().getActiveShell(), varInfo, newFileModel);
			newDialog.open();
		}
	...
	public static void execute(ProjectExplorerTreeNodeModel newFileModel, int fileFormat, String delimiter) {
		...
		ArrayList<String> varInfo=dataManipulationManager.getVarInfoFromFile(absoluteFilePath);
		System.out.println(newFileModel.getProjectFile().getName());
		ReshapeLongToWideDialog newVarDlg = new ReshapeLongToWideDialog(Display.getCurrent().getActiveShell(), varInfo, newFileModel);
		newVarDlg.open();
	}
}
