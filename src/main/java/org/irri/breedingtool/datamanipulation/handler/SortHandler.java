...
import org.irri.breedingtool.datamanipulation.dialog.SortDataDialog;
...
public static String PackageName = "Star";
	...
	public static void execute() {
		...
		SortDataDialog newDialog = new SortDataDialog(Display.getCurrent().getActiveShell(), varInfo, newFileModel);
		newDialog.open();
		}
	...
	public static void execute(ProjectExplorerTreeNodeModel newFileModel, int fileFormat, String delimiter) {
		...
		ArrayList<String> varInfo=dataManipulationManager.getVarInfoFromFile(absoluteFilePath);
		System.out.println(newFileModel.getProjectFile().getName());
		SortDataDialog newVarDlg = new SortDataDialog(Display.getCurrent().getActiveShell(), varInfo, newFileModel);
		newVarDlg.open();
	}
}
