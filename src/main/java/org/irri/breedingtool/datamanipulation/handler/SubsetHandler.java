...
import org.irri.breedingtool.datamanipulation.dialog.SubsetDialog;
...
public class SubsetHandler {
	...
	public static void execute() {
		try{
			...
			SubsetDialog newDialog = new SubsetDialog(Display.getCurrent().getActiveShell(), varInfo, newFileModel);
			newDialog.open();
		}
	...
	public static void execute(ProjectExplorerTreeNodeModel newFileModel, int fileFormat, String delimiter) {
		...
		ArrayList<String> varInfo=dataManipulationManager.getVarInfoFromFile(absoluteFilePath);
		System.out.println(newFileModel.getProjectFile().getName());
		SubsetDialog newVarDlg = new SubsetDialog(Display.getCurrent().getActiveShell(), varInfo, newFileModel);
		newVarDlg.open();
	}
}
