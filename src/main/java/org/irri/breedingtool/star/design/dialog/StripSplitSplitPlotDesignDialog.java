...
public class StripSplitSplitPlotDesignDialog extends Dialog {
	...
	protected void okPressed(){  	
		...
		if(txtVerticalFactorName.getText().contains(" ") || txtHorizontalFactorName.getText().contains(" ")){
			MessageDialog.open(SWT.ERROR, this.getShell(), "Error", "Factor Name must not contain space", SWT.ERROR);
			return;
		}
		if(txtSubPlotFactorName.getText().contains(" ") || txtSubSubPlotFactorName.getText().contains(" ")){
			MessageDialog.open(SWT.ERROR, this.getShell(), "Error", "Factor Name must not contain space", SWT.ERROR);
			return;
		}
		...
		if(txtSubPlotFactorID.getText().contains(" ") || txtSubSubPlotFactorID.getText().contains(" ")){
			MessageDialog.open(SWT.ERROR, this.getShell(), "Error", "Factor ID must not contain space", SWT.ERROR);
			return;
		}
		...
		if ((txtFieldRows.getSelection()  % (txtHorizontalFactorLevels.getSelection() * txtRows.getSelection() * txtRowsPerSub.getSelection() )) !=0 ){
			MessageDialog.openError(getShell(), "Error", "Number of field rows must be divisible by the product of the levels of the horizontal, number of plots along the length of each VxH plot and number of plots along the length of each sub plot."); 
			return ;
		}
		...
		if (txtRowsPerSub.getSelection()> txtSubSubPlotFactorLevels.getSelection()){
			MessageDialog.openError(getShell(), "Error", "Number of plots along the length of each sub plot should divide the levels of the sub-sub plot factor."); 
			return ;
		}
