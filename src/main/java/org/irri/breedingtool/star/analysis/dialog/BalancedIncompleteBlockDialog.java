...
import org.irri.breedingtool.star.analysis.anova.dialog.ContrastDialog;
import org.irri.breedingtool.star.analysis.anova.dialog.PostHocDialog;
import org.irri.breedingtool.utility.DialogFormUtility;
import org.irri.breedingtool.utility.StarAnalysisUtilities;

public class BalancedIncompleteBlockDialog extends Dialog {
...
//	private String[] postHocOpts = null;
	 ...
	 * @param parentShell
	 */
	public BalancedIncompleteBlockDialog(Shell parentShell) {
		super(parentShell);
	...
	protected void configureShell(Shell shell) {
		...
		shell.setText("Randomized Complete Block Design (RCBD) : "
	...
	protected Control createDialogArea(Composite parent) {
		...
		gd_btnResponseVariable.heightHint = 26;
		...
		group.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, true, 2, 3));
		group.setText("Set");
		...
		btnPerformCombined.setEnabled(false);
		GridData gd_btnPerformCombined = new GridData(SWT.FILL, SWT.CENTER,
				true, false, 2, 1);
		gd_btnPerformCombined.widthHint = 178;
		btnPerformCombined.setLayoutData(gd_btnPerformCombined);
		...
		gd_btnAddCombinedAnalysis.horizontalIndent = 2;
		...
		txtCombinedAnalysis = new Text(group, SWT.BORDER | SWT.READ_ONLY);
		txtCombinedAnalysis.setEditable(true);
		txtCombinedAnalysis.setForeground(SWTResourceManager.getColor(SWT.COLOR_BLACK));
		txtCombinedAnalysis.setBackground(SWTResourceManager
				.getColor(SWT.COLOR_WHITE));
		GridData gd_txtCombinedAnalysis = new GridData(SWT.FILL, SWT.CENTER,
				false, false, 1, 1);
		gd_txtCombinedAnalysis.widthHint = 237;
		...
		lblFactors_1.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));
		lblFactors_1.setText("Treatment");
		...
		txtTrmt = new Text(cmpModel, SWT.BORDER);
		txtTrmt.setEditable(false);
		txtTrmt.setBackground(SWTResourceManager.getColor(SWT.COLOR_WHITE));
		GridData gd_txtTrmt = new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1);
		gd_txtTrmt.heightHint = 18;
		txtTrmt.setLayoutData(gd_txtTrmt);
		new Label(cmpModel, SWT.NONE);
...
//				postHocOpts = null;
		...
		listManager.initializeSingleSelectionList(lstFactorVariables, txtBlock,btnMove, btnBlock);
		listManager.initializeSingleSelectionList(lstFactorVariables,txtTrmt,btnFactors);
		listManager.initializeVariableMoveList(lstNumericVariables,lstFactorVariables, btnMove, filePath);
		listManager.initializeSingleSelectionList(lstFactorVariables,txtCombinedAnalysis, btnMove, btnAddCombinedAnalysis);
		listManager.initializeSingleButtonList(lstNumericVariables,lstResponse, btnMove, btnResponseVariable);
...
//			postHocOpts = null;
			...
			if (dlgPostHoc == null || dlgPostHoc.getShell().isDisposed()) {
				dlgPostHoc = new PostHocDialog(this.getShell(), preHocResult,
						...
						design, btnPerformCombined.getSelection(),false);
				dlgPostHoc.open();
...
//						new String[] {txtTrmt.getText()}, filePath, outputFolderPath,
...
//		} 
	...
	protected void okPressed() {
		String[] respvar = lstResponse.getItems();
		String treatment = txtTrmt.getText();
		String block = txtBlock.getText();
		String[] method = null;
		boolean descriptive = btnDisplay1.getSelection();
		boolean normality = btnDisplay2.getSelection();
		boolean homogeneity = btnDisplay3.getSelection();
		double sig  = ((double)txtSignificantLevel.getSelection()/100);
		...
		if (validateCmpModel()) {
			...
			if (outputFolderPath == null)
				outputFolderPath = StarAnalysisUtilities.createOutputFolder(
					...
					preHocResult = ProjectExplorerView.rJavaManager.getSTARAnalysisManager().doBIBD(
...
//					new String[] {txtTrmt.getText()}, filePath));
