...
public class GriffingMethodDialog extends Dialog {
	...
	void initializeForm(){
		...
		dlgManager.initializeSingleSelectionList(factorVarList, txtP1Var, moveBtn,btnAddP1, new TextVarValidator(txtP1Var, lblP1));
		dlgManager.initializeSingleSelectionList(factorVarList, txtP2Var, moveBtn,btnAddP2, new TextVarValidator(txtP2Var, lblP2));
	...
	protected void okPressed(){
		...
		//check if required fields are empty
			...
			MessageDialog.openError(getShell(), "Error", "Please add a P1 factor.");
		}
		else if(txtP2Var.getText().isEmpty()){
			MessageDialog.openError(getShell(), "Error", "Please add a P2 factor.");
		}
			...
			try{
				if( Double.parseDouble(significanceLevel.getText())<=0 || Double.parseDouble(significanceLevel.getText())>=1){//check level if 0<levelOfSignificance<1
					...
					if(analysisType.equals("GriffingI")){
						if(performPerSiteRadio.getSelection())ProjectExplorerView.rJavaManager.getPbToolAnalysisManager().doDiallel1Test(dataFileName, newFileName, outputFolder.getAbsolutePath().replaceAll("\\\\+", "/")+"/", design, respvars, p1, p2, rep, block, row, column, cross, individual, environment, alpha);
						else ProjectExplorerView.rJavaManager.getPbToolAnalysisManager().doDiallel1METest(dataFileName, newFileName,outputFolder.getAbsolutePath().replaceAll("\\\\+", "/")+"/", design, respvars, p1, p2, rep, block, row, column, cross, individual, environment, alpha);
					}
					else if(analysisType.equals("GriffingII")){
						if(performPerSiteRadio.getSelection())ProjectExplorerView.rJavaManager.getPbToolAnalysisManager().doDiallel2Test(dataFileName, newFileName, outputFolder.getAbsolutePath().replaceAll("\\\\+", "/")+"/",design, respvars, p1, p2, rep, block, row, column, cross, individual, environment, alpha);
						else ProjectExplorerView.rJavaManager.getPbToolAnalysisManager().doDiallel2METest(dataFileName, newFileName,outputFolder.getAbsolutePath().replaceAll("\\\\+", "/")+"/", design, respvars, p1, p2, rep, block, row, column, cross, individual, environment, alpha);
					}
					else if(analysisType.equals("GriffingIII")){
						if(performPerSiteRadio.getSelection())ProjectExplorerView.rJavaManager.getPbToolAnalysisManager().doDiallel3Test(dataFileName, newFileName, outputFolder.getAbsolutePath().replaceAll("\\\\+", "/")+"/",design, respvars, p1, p2, rep, block, row, column, cross, individual, environment, alpha);
						else ProjectExplorerView.rJavaManager.getPbToolAnalysisManager().doDiallel3METest(dataFileName, newFileName,outputFolder.getAbsolutePath().replaceAll("\\\\+", "/")+"/", design, respvars, p1, p2, rep, block, row, column, cross, individual, environment, alpha);
					}
					else{
						if(performPerSiteRadio.getSelection())ProjectExplorerView.rJavaManager.getPbToolAnalysisManager().doDiallel4Test(dataFileName, newFileName, outputFolder.getAbsolutePath().replaceAll("\\\\+", "/")+"/",design, respvars, p1, p2, rep, block, row, column, cross, individual, environment, alpha);
						else ProjectExplorerView.rJavaManager.getPbToolAnalysisManager().doDiallel4METest(dataFileName, newFileName,outputFolder.getAbsolutePath().replaceAll("\\\\+", "/")+"/", design, respvars, p1, p2, rep, block, row, column, cross, individual, environment, alpha);
					}
					rInfo.close();
