package org.irri.breedingtool.pbtools.analysis.generationmean.handler;
...
import org.irri.breedingtool.pbtools.analysis.dialog.QTLAnalysisDialog;
import org.irri.breedingtool.pbtools.analysis.dialog.SelectionIndexDialog;
...
import org.irri.breedingtool.pbtools.analysis.generationmean.dialog.SummaryStatisticsDialog;
...
public class SummaryStatisticsDialogHandler {
	ProjectTreeComponent	projectTreeComponent ;
	SummaryStatisticsDialog newDialog;
	@Inject
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		dataManipulationManager.saveFileChanges(new File(absoluteFilePath), DataManipulationManager.getActiveTable(absoluteFilePath),  DataManipulationManager.getActiveDataDelimiter(absoluteFilePath));
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("GMASummaryStat", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("GMASummaryStat",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					newDialog = new SummaryStatisticsDialog(parent, "GMASummaryStat", activeFile);
					newDialog.open();
				}
			}
