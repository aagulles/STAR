...
public class SingleEnvironmentHandler {
	ProjectTreeComponent	projectTreeComponent ;
	SingleEnvironmentDialog singleEnvironmentDialog;
	@Inject
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		dataManipulationManager.saveFileChanges(new File(absoluteFilePath), DataManipulationManager.getActiveTable(absoluteFilePath),  DataManipulationManager.getActiveDataDelimiter(absoluteFilePath));
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("SingleEnv", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("SingleEnv",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					singleEnvironmentDialog = new SingleEnvironmentDialog(parent, "SingleEnv", activeFile);
					singleEnvironmentDialog.open();
				}
			}
