...
public class NorthCarolinaExperimentIHandler {
	...
	public void execute(final Shell parent, EModelService service,  MWindow window) {
		...
		try{//check if there's an active File first
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("NCI", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("NCI",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					matingDesignDialog = new NorthCarolinaExperimentsDialog(parent, "NCI", activeFile);
