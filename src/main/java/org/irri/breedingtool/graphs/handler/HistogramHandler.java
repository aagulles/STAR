...
public class HistogramHandler {
	...
	public static void execute() {
		try{
			...
			try{
				if(WindowDialogControlUtil.doesWindowDialogForFileExist("Histogram", activeFile.getAbsolutePath())){//if the Dialog was just Hidden
					WindowDialogControlUtil.showWindowDialog("Histogram",activeFile.getAbsolutePath());
				}
				else{//If the Dialog has been closed, create another
					HistogramDialog newDialog = new HistogramDialog(Display.getCurrent().getActiveShell(), "Histogram", newFileModel.getProjectFile());
					newDialog.open();
				}
			}
